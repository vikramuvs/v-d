...
export async function getStaticProps() {
  const iv = await fetch(
    "http://ec2-52-39-2-31.us-west-2.compute.amazonaws.com:8080/api/v1/ind_visits/35"
  );
  const ivJson = await iv.json();

  const ssw = await fetch(
    "http://ec2-52-39-2-31.us-west-2.compute.amazonaws.com:8080/api/v1/ssw_fdp/35"
  );
  const sswJson = await ssw.json();

  const seo = await fetch(
    "http://ec2-52-39-2-31.us-west-2.compute.amazonaws.com:8080/api/v1/events_org/35"
  );
